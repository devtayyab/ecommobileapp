  ...
  return (
    <View style={{ paddingVertical: scale(50) }}>
      ...
      <View style={{ paddingVertical: scale(10) }}>
        <CustomInput containerStyle={{ backgroundColor: 'transparent' }} value={cardinfo?.cardname} label="Name on card" onChangeText={(v)=>hanndleChange('cardname' , v)} />
      </View>
      <View style={{ paddingVertical: scale(10) }}>
        <CustomInput containerStyle={{ backgroundColor: 'transparent' }} value={cardinfo?.cardnumber} label="Number on card" onChangeText={(v)=>hanndleChange('cardnumber' , v)} IconRight={() => <Feather name="credit-card" color={appColors.primary} size={scale(20)} />} />
      </View>

      <View style={{ paddingVertical: scale(10) }}>
        <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center' }}>
          <CustomInput containerStyle={{ backgroundColor: 'transparent' }} value={cardinfo?.expirydate} label="Expiry Date" onChangeText={(v)=>hanndleChange('expirydate' ,v)}/>
          <CustomInput containerStyle={{ backgroundColor: 'transparent' }} value={cardinfo?.cvv} onChangeText={(v)=>hanndleChange('cvv' ,v)} label="CVV" />
        </View>
      </View>
